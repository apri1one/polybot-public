...
export class PolymarketTrader extends EventEmitter {
    ...
    private marketInfoCache = new Map<string, { data: { tickSize: number; negRisk: boolean; tokens: { tokenId: string; outcome: string }[] }; timestamp: number }>();
    // 签名预检缓存: key = `${negRisk}` (签名正确性仅取决于 negRisk → 合约地址)
    private preflightCache = new Map<string, { success: boolean; timestamp: number }>();
    // tokenId -> feeRateBps 缓存 (10分钟 TTL，费率不常变)
    ...
    async placeOrder(input: PolyOrderInput): Promise<PolyOrderResult> {
        ...
        try {
            ...
            const requestedExpiresAt = toFutureTimestampMs(input.expiresAt);
            const useGtdExpiry = orderType !== 'IOC' && requestedExpiresAt !== null;
            const apiOrderType = useGtdExpiry ? 'GTD' : orderType;

            if (requestedExpiresAt !== null && requestedExpiresAt <= Date.now()) {
                const error = `Invalid expiresAt: ${requestedExpiresAt}`;
                console.error(`[PolymarketTrader] ${error}`);
                return { success: false, error };
            }

            if (orderType === 'IOC' && requestedExpiresAt !== null) {
                console.warn('[PolymarketTrader] expiresAt is ignored for IOC orders');
            }
            ...
            // 获取市场费率 (NCAAB/Serie A 等市场有 taker 费)
            const feeRateBps = input.feeRateBps ?? await this.getFeeRate(input.tokenId);

            // 构建订单
            const salt = Math.round(Math.random() * Date.now());
            // GTC 和 IOC 订单的 expiration 都必须为 0
            // 只有 GTD (Good-Till-Date) 订单才需要设置过期时间
            const expiration = useGtdExpiry
                ? BigInt(Math.floor((requestedExpiresAt! / 1000) + GTD_SECURITY_BUFFER_SECONDS))
                : BigInt(0);
            ...
            if (this.telegram) {
                ...
                this.telegram.alertOrder({
                    ...
                    role: apiOrderType === 'IOC' ? 'Taker' : 'Maker',
                    orderHash: orderId,
                    timestamp: Date.now(),
                }).catch(() => {});
            }
    ...
    async getOrderStatus(orderId: string): Promise<PolyOrderStatus | null> {
        try {
            ...
            return {
                status: normalizePolymarketOrderStatus(data.status),
     ...
     * 取消所有挂单
     */
    async cancelAllOrders(options?: { timeoutMs?: number }): Promise<boolean> {
        ...
        try {
            ...
            try {
                ...
                if (e?.name === 'AbortError') {
                    console.warn('[PolyTrader] ⚠️ Timeout cancelling all orders');
                    return false;
                }
                throw e;
            } finally {
            ...
            if (res.ok) {
                console.log('[PolyTrader] ✅ All orders cancelled');
            } else {
                console.warn(`[PolyTrader] ⚠️ Cancel all orders failed: ${res.status} ${res.statusText}`);
            }

            return res.ok;
        } catch (e: any) {
            console.error(`[PolyTrader] ❌ cancelAllOrders error: ${e?.message || e}`);
            return false;
        }
    }
    ...
    async getBalance(): Promise<number> {
        try {
            const ba = await this.getBalanceAllowance();
            return ba.balance;
        } catch {
            return 0;
        }
     ...
     * 获取余额和 allowance
     */
    async getBalanceAllowance(): Promise<{ balance: number; allowance: number }> {
        const signPath = '/balance-allowance';
        const queryParams = '?asset_type=COLLATERAL&signature_type=2';
        const headers = this.buildHeaders('GET', signPath);

        const res = await fetch(`${CLOB_BASE_URL}${signPath}${queryParams}`, { headers });
    ...
    /**
     * 刷新 CLOB 后端的 balance/allowance 缓存
     * SDK 实现: GET /balance-allowance/update?asset_type=COLLATERAL&signature_type=2
     * 注意: HMAC 签名只用路径部分，query params 不参与签名
     */
    async updateBalanceAllowance(): Promise<boolean> {
        try {
            const signPath = '/balance-allowance/update';
            const queryParams = '?asset_type=COLLATERAL&signature_type=2';
            const headers = this.buildHeaders('GET', signPath);
            ...
            if (res.ok) {
                console.log('[PolymarketTrader] Balance/allowance cache refreshed');
            } else {
                console.warn(`[PolymarketTrader] updateBalanceAllowance failed: ${res.status}`);
            }

            return res.ok;
        } catch (e: any) {
            console.warn(`[PolymarketTrader] updateBalanceAllowance error: ${e?.message}`);
            return false;
        }
    }
    ...
    async preflightCheck(input: {
        ...
        // 缓存命中: 签名正确性仅取决于 negRisk (决定合约地址)，10分钟内不重复验证
        const cacheKey = `${input.negRisk}`;
