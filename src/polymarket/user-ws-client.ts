...
export class PolymarketUserWsClient {
    ...
    private subscribeAllMarkets = true;
    ...
    constructor(auth: UserWsAuth, config: PolymarketUserWsClientConfig = {}) {
        ...
        this.replaceMarkets(config.markets ?? []);
    }
    ...
    getSubscribedMarkets(): string[] {
        return this.subscribeAllMarkets ? [] : Array.from(this.subscribedMarkets);
    }

    replacesAllMarkets(): boolean {
        return this.subscribeAllMarkets;
    }

    replaceMarkets(markets: string[]): void {
        const normalized = normalizeMarkets(markets);
        this.subscribeAllMarkets = normalized.length === 0;
        this.subscribedMarkets = new Set(normalized);
    }

    async resubscribeMarkets(markets: string[]): Promise<void> {
        const normalized = normalizeMarkets(markets);
        const wasAllMarkets = this.subscribeAllMarkets;
        const previous = this.getSubscribedMarkets();

        this.replaceMarkets(normalized);

        if (!this.isConnected || !this.ws || this.ws.readyState !== WebSocket.OPEN) {
            return;
        }

        if (wasAllMarkets !== this.subscribeAllMarkets) {
            try {
                this.ws.close(1000, 'Resubscribe markets');
            } catch {
        ...
        if (this.subscribeAllMarkets) {
            return;
        }

        const previousSet = new Set(previous);
        const nextSet = new Set(normalized);
        const toSubscribe = normalized.filter(market => !previousSet.has(market));
        const toUnsubscribe = previous.filter(market => !nextSet.has(market));
    ...
    private sendSubscription(operation?: 'subscribe' | 'unsubscribe', markets?: string[]): void {
        if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
            throw new Error('User WS is not connected');
        }

        const payload: Record<string, unknown> = {
            type: 'USER',
            markets: normalizeMarkets(markets ?? this.getSubscribedMarkets()),
            auth: {
