...
export class LogQueryService {
    ...
    constructor(logsBaseDir?: string) {
        this.logsBaseDir = logsBaseDir || path.join(process.cwd(), 'data', 'logs', 'tasks');
    }
    ...
    getTaskTimeline(taskId: string): TaskTimeline | null {
        ...
        // 构建时间线事件
        const timelineEvents: TaskTimelineEvent[] = events.map((event: any) => {
            let detail = '';
    ...
    getFailures(days: number = 7): FailedTask[] {
        ...
        for (const taskId of taskDirs) {
            ...
            const eventsPath = path.join(taskDir, 'events.jsonl');
            const events = readJsonl<any>(eventsPath);
            const failedEvent = events.find((e: any) => e.type.includes('FAILED'));
